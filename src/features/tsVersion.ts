...
function getConfigUseWorkspaceTsdk() {
  return workspace.getConfiguration('volar').get<boolean>('alpine.useWorkspaceTsdk', false);
}

// Memo: volar.diagnostics.tsLocale for coc-volar
function getConfigDiagnosticsTsLocale() {
  return workspace.getConfiguration('volar').get<string>('alpine.diagnostics.tsLocale', 'en');
}
